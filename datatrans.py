...
class DataTrans:
    ...
    def load_profiles(self):
        ...
        if not all(
            ...
            raise ValueError(
                "Missing required source database connection information in trans_settings.yml"
            )
        ...
        if not all(
            ...
            raise ValueError(
                "Missing required target database connection information in trans_settings.yml"
            )
    ...
    def connect_to_dws(self):
        try:
            conn = psycopg2.connect(
                database=self.tag_dbname,
                user=self.tag_user,
                password=self.tag_pass,
                host=self.tag_host,
                port=self.tag_port,
            )
            cur = conn.cursor()
    ...
    def write_ddl_to_sql_file(self, ddls):
        sql_file_path = os.path.join(
            self.current_directory, "dws_create_stg_tables.sql"
        )
